...
class Client:
    ...
    async def pull_chart(
        ...
        *,
        ...
        try:
            ...
            # unpacked chart. We want the actual chart directory
    ...
    async def get_current_revision(
        ...
        return ReleaseRevision._from_status(
            await self._command.status(release_name, namespace=namespace),
            self._command,
        )
        ...
        *,
        ...
        try:
            ...
            if current_revision.status in {
                # If the release is stuck in pending-install, there is nothing to
                # rollback to.
                # Instead, we have to uninstall the release and try again
                ReleaseRevisionStatus.PENDING_INSTALL,
                # If the release is stuck in uninstalling, we need to complete the
                # uninstall
    ...
    async def ensure_release(
        ...
        *values: t.Dict[str, t.Any],
        ...
        It the release must be rolled back or deleted in order to be proceedable, this
        method will ensure that happens. It will also only make a new release if the
        chart and/or values have changed.
        """
        values = mergeconcat(*values) if values else {}
